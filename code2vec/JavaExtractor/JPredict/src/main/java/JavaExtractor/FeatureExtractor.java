...
public class FeatureExtractor {
	...
	public ArrayList<ProgramFeatures> extractFeatures(String code,int level) throws ParseException, IOException {
		CompilationUnit compilationUnit = parseFileWithRetries(code);
		FunctionVisitor functionVisitor = new FunctionVisitor();
		...
		ArrayList<MethodContent> methodContents =new ArrayList<>();

		int methodLength = 0;

		if (level==1){
			for (MethodContent methodContent:methods){
				methodLength = methodContent.getName().length();
				if (methodLength<4){
		...
		if (level==2){
			for (MethodContent methodContent:methods){
				methodLength = methodContent.getName().length();
				if (methodLength>3 && methodLength<28){
					methodContents.add(methodContent);
				}
			}
		}
		if (level==3){
			for (MethodContent methodContent:methods){
				methodLength = methodContent.getName().length();
				if (methodLength>27 && methodLength<52){
					methodContents.add(methodContent);
				}
			}
		}
		if (level==4){
			for (MethodContent methodContent:methods){
				methodLength = methodContent.getName().length();
				if (methodLength>51 && methodLength<80){
					methodContents.add(methodContent);
				}
			}
		}
		...
		ArrayList<ProgramFeatures> programs = generatePathFeatures(methodContents);
